"""Train v10: v5 + bounded learnable gamma (gamma_max=0.5)"""
...
from models_variants.tri_branch_v10.variant_model import TriBranch_USAD_v10
...
def main():
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], "tri_branch_v10"); ensure_dir(save_dir)
    ...
    model = TriBranch_USAD_v10(
        ...
        gamma_mode="bounded_learnable", gamma_max=0.5, gamma_init=0.30, gate_scale=1.0,
    ).to(device)
    print(f"v10: v5 + bounded gamma (max=0.5, init=0.30), params={sum(p.numel() for p in model.parameters()):,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        eg = model.encoder.gated_fusion.get_gamma().item()
        print(f"Epoch {epoch:03d} (P{phase}) | train {train_loss:.6f} | val {val_loss:.6f} | gamma {eg:.4f}")
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss, "gamma": eg})
    ...
    final_gamma = model.encoder.gated_fusion.get_gamma().item()
    ...
    print(f"\n=== v10 (bounded gamma={final_gamma:.4f}) ===")
