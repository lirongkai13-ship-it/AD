"""
Tri-Branch USAD v6 — v5 + Temporal FFN Block.

Minimal change: adds lightweight FFN after MHA+LN in Branch 3.
Pipeline: proj_in → +pos_emb → MHA+LN → FFN+LN → mean_pool → expand → vector gate
"""
import torch
...
class GlobalTemporalAttentionBranch(nn.Module):
    """
    v6: v5 + FFN block after MHA.
    Pipeline: proj_in → +pos_emb → MHA+LN → FFN+LN → mean_pool → expand
    """

    def __init__(self, num_variables=51, window_size=60, d=32,
                 n_heads=4, dropout=0.1, d_ff=64, max_window_size=128):
        super().__init__()
        ...
        self.norm1 = nn.LayerNorm(d); self.drop1 = nn.Dropout(dropout)

        # 4. FFN Block (v6 NEW)
        self.ffn = nn.Sequential(
            nn.Linear(d, d_ff), nn.ReLU(), nn.Dropout(dropout), nn.Linear(d_ff, d))
        self.norm2 = nn.LayerNorm(d); self.drop2 = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        # 2. Position Embedding
        T_emb = T_emb + self.pos_emb[:, :W, :]

        # 3. MHA + residual + LN
        T_attn_raw, _ = self.attn(T_emb, T_emb, T_emb)
        T_attn = self.norm1(T_emb + self.drop1(T_attn_raw))  # [B, W, d]

        # 4. FFN + residual + LN (v6 NEW)
        T_ffn = self.norm2(T_attn + self.drop2(self.ffn(T_attn)))  # [B, W, d]

        # 5. Mean pooling
        g_global = T_ffn.mean(dim=1)  # [B, d]

        # 6. Expand
        return g_global.unsqueeze(1).expand(B, N, d), T_emb, T_ffn
...
class TriBranchEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.encoder_mode == "tri_branch_residual_gate" and self.global_temp is not None:
            h_global, T_emb, T_out = self.global_temp(x)
            ...
            if not self._debug_done and self.training:
                ...
                print(f"\n[v6 DEBUG] pos_emb mean={pe.mean().item():.4f} std={pe.std().item():.4f}")
                print(f"  FFN d_ff={self.global_temp.ffn[0].out_features}")
                print(f"  T_emb norm={T_emb.norm().item():.2f} T_out norm={T_out.norm().item():.2f}")
...
class TriBranch_USAD_v6(nn.Module):
