"""Train v7: v5 + Recent-aware Pooling"""
...
from models_variants.tri_branch_v7.variant_model import TriBranch_USAD_v7
...
def main():
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], "tri_branch_v7"); ensure_dir(save_dir)
    ...
    if dcfg.get("timestamp_col") in nfd.columns: nfd.drop(columns=[dcfg["timestamp_col"]], inplace=True)
    if dcfg.get("label_col") in nfd.columns: nfd.drop(columns=[dcfg["label_col"]], inplace=True)
    ...
    print(f"v7: v5 + recent-aware pooling, params={sum(p.numel() for p in model.parameters()):,}")

    optimizer = torch.optim.Adam(model.parameters(), lr=float(cfg["train"]["lr"]),
                                 weight_decay=float(cfg["train"]["weight_decay"]))
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=0.5, patience=3, min_lr=1e-6)
    mse = nn.MSELoss()
    epochs = int(cfg["train"]["epochs"]); early_stop = int(cfg["train"]["early_stop_patience"])
    ...
    for epoch in range(1, epochs + 1):
        ...
        gv = model.encoder.gated_fusion.gamma.item()
        bv = F.softplus(model.encoder.global_temp.raw_beta).item()
        print(f"Epoch {epoch:03d} (P{phase}) | train {train_loss:.6f} | val {val_loss:.6f} | gamma {gv:.4f} | beta {bv:.4f}")
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss, "gamma": gv, "beta": bv})
    ...
    save_json({"history": history, "best_val_loss": float(best_val)}, os.path.join(save_dir, "train_history.json"))
    ...
    for batch in tqdm(test_loader, desc="eval test"):
        with torch.no_grad():
            x = batch["x"].to(device); recon = model.forward_eval(x, static_ei)
    ...
    final_gamma = model.encoder.gated_fusion.gamma.item()
    final_beta = F.softplus(model.encoder.global_temp.raw_beta).item()
    metrics = {"threshold": threshold, "raw": raw_m, "point_adjust": pa_m,
               "train_time_min": train_time/60, "final_gamma": final_gamma, "recent_beta": final_beta}
    save_json(metrics, os.path.join(save_dir, "metrics.json"))
    print(f"\n=== v7 (v5+recent, gamma={final_gamma:.4f} beta={final_beta:.4f}) ===")
    print(f"F1={raw_m['f1']:.4f} AUC={raw_m.get('roc_auc','?')}")
