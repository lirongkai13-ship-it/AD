"""
Tri-Branch USAD v7 — v5 + Recent-aware Pooling.

Minimal change: adds recent-aware pooling on top of v5's mean pooling.
g_global = LayerNorm(g_mean + beta * g_recent)
beta = softplus(raw_beta), init raw_beta=-4.0 (beta≈0, degrades to v5)
"""
import torch
...
# === Branch 3 v7: v5 + Recent-aware Pooling ===
class GlobalTemporalAttentionBranch(nn.Module):
    """
    v7: v5 + recent-aware pooling.
    g_global = LayerNorm(g_mean + beta * g_recent)
    beta = softplus(raw_beta), init raw_beta=-4.0 (beta≈0.018, near v5)
    """

    def __init__(self, num_variables=51, window_size=60, d=32,
                 n_heads=4, dropout=0.1, recent_k=10, max_window_size=128):
        super().__init__()
        self.d = d; self.window_size = window_size; self.recent_k = recent_k
        ...
        # Recent-aware pooling (v7 NEW)
        self.raw_beta = nn.Parameter(torch.tensor(-4.0))  # softplus(-4) ≈ 0.018
        self.norm_pool = nn.LayerNorm(d)
    ...
    def forward(self, x):
        ...
        # Pooling: global mean + recent mean
        g_mean = T_attn.mean(dim=1)  # [B, d]
        T_recent = T_attn[:, -self.recent_k:, :]  # [B, k, d]
        g_recent = T_recent.mean(dim=1)  # [B, d]
        beta = F.softplus(self.raw_beta)  # always ≥ 0
        g_global = self.norm_pool(g_mean + beta * g_recent)  # [B, d]

        return g_global.unsqueeze(1).expand(B, N, d), T_emb, T_attn, beta
...
class TriBranchEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.encoder_mode == "tri_branch_residual_gate" and self.global_temp is not None:
            h_global, T_emb, T_attn, beta = self.global_temp(x)
            ...
            if not self._debug_done and self.training:
                ...
                print(f"\n[v7 DEBUG] recent_k={self.global_temp.recent_k} beta={beta.item():.4f}")
...
# === Full Model ===
class TriBranch_USAD_v7(nn.Module):
