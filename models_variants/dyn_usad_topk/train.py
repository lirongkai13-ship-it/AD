"""训练: Dynamic USAD + Top-K Edge (k sweep)"""
...
from variant_model import DynTopK_USAD
...
def main(top_k=10):
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], f"dyn_usad_topk_k{top_k}")
    ensure_dir(save_dir)
    ...
    model = DynTopK_USAD(
        info["num_variables"], int(cfg["data"]["window_size"]), static_ei, top_k,
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch:03d} (P{phase}, k={top_k}) | train {train_loss:.6f} | val {val_loss:.6f}")
    ...
    save_json({"history": history, "best_val_loss": float(best_val), "top_k": top_k},
    ...
    metrics = {"top_k": top_k, "threshold": threshold, "raw": raw_m, "point_adjust": pa_m,
               "train_time_min": train_time/60}
    save_json(metrics, os.path.join(save_dir, "metrics.json"))
    print(f"Top-K k={top_k}: F1={raw_m['f1']:.4f}  AUC={raw_m.get('roc_auc','?')}")
    return metrics
