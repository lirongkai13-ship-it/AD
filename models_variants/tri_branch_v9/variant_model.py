"""
Tri-Branch USAD v9 — v5 + Temporal CLS Token.

CLS token prepended to temporal sequence → MHA → CLS output as g_global.
Replaces mean pooling; keeps all else unchanged.
"""
import torch
...
# === Branch 3 v9: v5 + CLS Token ===
class GlobalTemporalAttentionBranch(nn.Module):
    """
    v9: CLS token prepended → MHA → CLS output as g_global.
    """
    ...
    def __init__(self, num_variables=51, window_size=60, d=32,
        ...
        self.pos_emb = nn.Parameter(torch.randn(1, max_window_size + 1, d) * 0.02)  # +1 for CLS
    ...
    def forward(self, x):
        ...
        # 2. Add position embedding (first W+1 positions reserved)
        T_emb = T_emb + self.pos_emb[:, 1:W+1, :]  # pos 1..W for tokens

        # 3. Prepend CLS token
        cls_tokens = self.cls_token.expand(B, -1, -1)  # [B, 1, d]
        cls_tokens = cls_tokens + self.pos_emb[:, :1, :]  # pos 0 for CLS
        T_with_cls = torch.cat([cls_tokens, T_emb], dim=1)  # [B, W+1, d]

        # 4. MHA
        T_attn_raw, attn_weights = self.attn(T_with_cls, T_with_cls, T_with_cls)
        T_attn = self.norm_attn(T_with_cls + self.drop_attn(T_attn_raw))  # [B, W+1, d]

        # 5. CLS output as global token
        g_global = T_attn[:, 0, :]  # [B, d]

        return g_global.unsqueeze(1).expand(B, N, d), T_emb, T_attn, attn_weights
...
class TriBranchEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.encoder_mode == "tri_branch_residual_gate" and self.global_temp is not None:
            h_global, T_emb, T_attn, attn_w = self.global_temp(x)
            ...
            if not self._debug_done and self.training:
                ...
                ct = self.global_temp.cls_token
                print(f"\n[v9 DEBUG] cls_token shape={list(ct.shape)} mean={ct.mean().item():.4f} std={ct.std().item():.4f}")
                print(f"  T_emb shape={list(T_emb.shape)} T_with_cls shape={list(T_attn.shape)}")
                print(f"  attn_weights shape={list(attn_w.shape)}")
...
class TriBranch_USAD_v9(nn.Module):
