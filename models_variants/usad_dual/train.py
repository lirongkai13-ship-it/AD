...
from variant_model import USAD_DualDecoder
...
def main():
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], "usad_dual")
    ensure_dir(save_dir)
    ...
    model = USAD_DualDecoder(
        info["num_variables"], int(cfg["data"]["window_size"]),
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="val", leave=False):
                x = batch["x"].to(device)
                r1 = model.forward_eval(x, edge_index)
    ...
    def eval_forward(batch):
        x = batch["x"].to(device)
        return model.forward_eval(x, edge_index)
