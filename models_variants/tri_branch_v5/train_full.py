"""Full-setting training for v5 (best model): stride=1, learnable gamma, pos_emb"""
...
def main():
    cfg = load_config('config_full.yaml')
    set_seed(int(cfg["train"]["seed"]))
    device = get_device(cfg["train"].get("device", "cuda"))
    save_dir = os.path.join(cfg["output"]["save_dir"], "tri_branch_v5_full"); ensure_dir(save_dir)
    ...
    print(f"v5 full-setting: learnable gamma + pos_emb, {sum(p.numel() for p in model.parameters()):,} params")
    print(f"stride=1, train={len(train_ds)} windows, val={len(val_ds)}, test={len(test_ds)}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"E{epoch}/{epochs}", leave=False):
        ...
        print(f"Epoch {epoch:03d} (P{phase}) | train {train_loss:.6f} | val {val_loss:.6f} | gamma {gv:.4f}")
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss, "gamma": gv})
    ...
    metrics = {"threshold": threshold, "raw": raw_m, "point_adjust": pa_m,
               "train_time_min": train_time/60, "final_gamma": final_gamma}
    save_json(metrics, os.path.join(save_dir, "metrics.json"))
    print(f"\n=== v5 FULL (gamma={final_gamma:.4f}) ===")
    print(f"F1={raw_m['f1']:.4f} P={raw_m['precision']:.4f} R={raw_m['recall']:.4f} AUC={raw_m.get('roc_auc','?')}")
