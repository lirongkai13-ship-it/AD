"""训练 tri_branch_v2 with learnable gamma"""
...
def main():
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], "tri_branch_v2_learnable_gamma")
    ensure_dir(save_dir)
    ...
    model = TriBranch_USAD_v2(
        ...
        gamma_mode="learnable",      # <-- LEARNABLE
        gamma_value=0.05,            # initial value
        gate_scale=1.0,
    ...
    print(f"Gamma mode: learnable, initial={model.encoder.gated_fusion.gamma.item():.4f}")
    ...
    print(f"\n=== Tri-Branch v2 (learnable gamma, final={final_gamma:.6f}) ===")
