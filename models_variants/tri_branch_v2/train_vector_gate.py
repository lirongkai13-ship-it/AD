"""Train v2 with vector gate + learnable gamma"""
...
def main():
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], "tri_branch_v2_vector_gate_learnable")
    ensure_dir(save_dir)
    ...
    model = TriBranch_USAD_v2(
        ...
        gate_type="vector",            # <-- VECTOR GATE
    ).to(device)
    print(f"Params: {sum(p.numel() for p in model.parameters()):,}")
    print(f"Gate type: vector [B,N,32], gamma: learnable")
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch:03d} (P{phase}) | train {train_loss:.6f} | val {val_loss:.6f} | gamma {gamma_val:.4f}")
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss, "gamma": gamma_val})
    ...
    print(f"\n=== v2 vector gate + learnable gamma (final={final_gamma:.4f}) ===")
