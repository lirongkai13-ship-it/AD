"""Dynamic USAD + Weighted Fusion: A = alpha * A_global + (1-alpha) * A_batch"""
import torch
...
class DynamicPearsonWeighted(nn.Module):
    """动态 Pearson + 全局图加权融合"""
    def __init__(self, n_vars, static_edge_index, global_corr, alpha=0.5, threshold=0.3):
        super().__init__()
        self.n_vars = n_vars
        self.alpha = alpha
        self.threshold = threshold
        self.register_buffer("static_ei", static_edge_index)
        self.register_buffer("global_corr", global_corr)  # [N, N] 训练集全局相关矩阵
    ...
    def forward(self, x):
        ...
        C_batch_avg = C_batch.abs().mean(dim=0)

        # 加权融合
        C_combined = self.alpha * self.global_corr + (1 - self.alpha) * C_batch_avg

        # 建边
        mask = (C_combined.abs() >= self.threshold) & (~torch.eye(n, dtype=torch.bool, device=x.device))
        if mask.any():
...
class DynWeighted_USAD(nn.Module):
    def __init__(self, num_variables, window_size, static_edge_index, global_corr,
                 alpha=0.5, hidden_dim=32, gat_heads=2, gru_hidden=32,
                 tcn_channels=32, tcn_blocks=1, dropout=0.2):
        super().__init__()
        dec_feat = window_size * num_variables
        self.dyn_graph = DynamicPearsonWeighted(num_variables, static_edge_index, global_corr, alpha)
