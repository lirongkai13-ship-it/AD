"""Train tri_branch_v3 with learnable gamma"""
...
from models_variants.tri_branch_v3.variant_model import TriBranch_USAD_v3
...
def main():
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], "tri_branch_v3")
    ensure_dir(save_dir)
    ...
    model = TriBranch_USAD_v3(
        ...
        use_pos_emb=True, use_relative_decay_bias=True, learnable_decay=True,
    ).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        gamma_val = model.encoder.gated_fusion.gamma.item()
        lambda_val = torch.nn.functional.softplus(model.encoder.global_temp.raw_lambda).item()
        print(f"Epoch {epoch:03d} (P{phase}) | train {train_loss:.6f} | val {val_loss:.6f} | gamma {gamma_val:.4f} | lambda {lambda_val:.4f}")
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss,
                        "gamma": gamma_val, "lambda_decay": lambda_val})
