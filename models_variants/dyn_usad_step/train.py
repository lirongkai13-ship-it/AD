"""训练: Dynamic USAD + Step Update (interval sweep)"""
...
from variant_model import DynStep_USAD


def main(step_interval=5):
    cfg = load_config(os.path.join(os.path.dirname(__file__), "..", "..", "config_dev.yaml"))
    set_seed(int(cfg["train"]["seed"]))
    device = get_device(cfg["train"].get("device", "cuda"))
    save_dir = os.path.join(cfg["output"]["save_dir"], f"dyn_usad_step_s{step_interval}")
    ensure_dir(save_dir)
    ...
    model = DynStep_USAD(
        info["num_variables"], int(cfg["data"]["window_size"]), static_ei, step_interval,
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch:03d} (P{phase}, step={step_interval}) | train {train_loss:.6f} | val {val_loss:.6f}")
    ...
    save_json({"history": history, "best_val_loss": float(best_val), "step_interval": step_interval},
    ...
    metrics = {"step_interval": step_interval, "threshold": threshold, "raw": raw_m,
               "point_adjust": pa_m, "train_time_min": train_time/60}
    save_json(metrics, os.path.join(save_dir, "metrics.json"))
    print(f"Step interval={step_interval}: F1={raw_m['f1']:.4f}  AUC={raw_m.get('roc_auc','?')}")
    return metrics

if __name__ == "__main__":
    for s in [3, 5, 10]:
        print(f"\n{'='*60}\n  Step Update: interval={s}\n{'='*60}")
        main(step_interval=s)
