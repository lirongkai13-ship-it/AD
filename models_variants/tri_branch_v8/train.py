"""Train v8: v5 + multi-scale mean pooling"""
...
from models_variants.tri_branch_v8.variant_model import TriBranch_USAD_v8
...
def main():
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], "tri_branch_v8"); ensure_dir(save_dir)
    ...
    print(f"v8: v5 + multi-scale pooling, params={sum(p.numel() for p in model.parameters()):,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        am = torch.sigmoid(model.encoder.global_temp.raw_alpha_mid).item()
        ar = torch.sigmoid(model.encoder.global_temp.raw_alpha_recent).item()
        print(f"Epoch {epoch:03d} (P{phase}) | train {train_loss:.6f} | val {val_loss:.6f} | gamma {gv:.4f} | a_mid {am:.4f} a_rec {ar:.4f}")
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss,
                        "gamma": gv, "alpha_mid": am, "alpha_recent": ar})
