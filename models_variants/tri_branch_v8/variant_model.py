"""
Tri-Branch USAD v8 — v5 + Multi-scale Temporal Mean Pooling.

g_global = fuse(g_full + alpha_mid * g_mid + alpha_recent * g_recent)
alpha_i = sigmoid(raw_alpha_i), init raw_alpha_i = -2.0 (alpha≈0.12, near v5)
"""
import torch
...
# === Branch 3 v8: v5 + Multi-scale Pooling ===
class GlobalTemporalAttentionBranch(nn.Module):
    """
    v8: v5 + multi-scale mean pooling (full, mid, recent).
    g_global = fuse(g_full + a_mid*g_mid + a_recent*g_recent)
    """

    def __init__(self, num_variables=51, window_size=60, d=32,
                 n_heads=4, dropout=0.1, mid_k=30, recent_k=10, max_window_size=128):
        super().__init__()
        self.d = d; self.window_size = window_size
        self.mid_k = mid_k; self.recent_k = recent_k
        ...
        # Multi-scale pooling weights (v8 NEW)
        self.raw_alpha_mid = nn.Parameter(torch.tensor(-2.0))
        self.raw_alpha_recent = nn.Parameter(torch.tensor(-2.0))
        # Fusion: concat(g_full, g_mid, g_recent) → g_global
        self.fuse_linear = nn.Linear(3 * d, d)
    ...
    def forward(self, x):
        ...
        # Multi-scale pooling
        g_full = T_attn.mean(dim=1)  # [B, d]
        g_mid = T_attn[:, -self.mid_k:, :].mean(dim=1)  # [B, d]
        g_recent = T_attn[:, -self.recent_k:, :].mean(dim=1)  # [B, d]

        a_mid = torch.sigmoid(self.raw_alpha_mid)
        a_recent = torch.sigmoid(self.raw_alpha_recent)

        g_global = self.fuse_linear(torch.cat([g_full, a_mid * g_mid, a_recent * g_recent], dim=-1))

        return g_global.unsqueeze(1).expand(B, N, d), T_emb, T_attn, a_mid, a_recent
...
class TriBranchEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.encoder_mode == "tri_branch_residual_gate" and self.global_temp is not None:
            h_global, T_emb, T_attn, a_mid, a_recent = self.global_temp(x)
            ...
            if not self._debug_done and self.training:
                ...
                print(f"\n[v8 DEBUG] alpha_mid={a_mid.item():.4f} alpha_recent={a_recent.item():.4f}")
                print(f"  T_emb norm={T_emb.norm().item():.2f} T_attn norm={T_attn.norm().item():.2f}")
                print(f"  gate mean={gate_val.mean().item():.4f} min={gate_val.min().item():.4f} max={gate_val.max().item():.4f}")
                print(f"  H_base norm={h_base.norm().item():.2f} H_global norm={h_global.norm().item():.2f}")
...
class TriBranch_USAD_v8(nn.Module):
