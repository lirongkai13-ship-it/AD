"""
方案D: 并行时空 USAD + 多尺度 TCN 时间分支 (dilation=1,2,4)

Shape 流程:
  Input:            X [B, 60, 51]
  Spatial Branch:   X → dyn_prior_graph → GATv2 → H_space [B, 51, 32]
  Temporal Branch:  X → per-var MultiScale TCN(k3,d1/2/4) → H_time [B, 51, 32]
...
# 3. 方案D: 多尺度 TCN 时间编码器
# ═══════════════════════════════════════════════════════════════
class MultiScaleTCNTemporalEncoder(nn.Module):
    """3个 TCN 分支 (dilation=1,2,4) → concat → pool → MLP → [B*N, d]"""
    def __init__(self, window_size=60, out_dim=32, branch_channels=8, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        h1 = self.pool(h1).squeeze(-1)  # [B*N, bc]
        h2 = self.pool(h2).squeeze(-1)
        h3 = self.pool(h3).squeeze(-1)
        h = torch.cat([h1, h2, h3], dim=-1)  # [B*N, 3*bc]
        return self.fuse(h)
...
# ═══════════════════════════════════════════════════════════════
# 6. ParallelSpatioTemporalEncoder (方案D版)
# ═══════════════════════════════════════════════════════════════
class ParallelSpatioTemporalEncoder(nn.Module):
    def __init__(self, num_variables=51, window_size=60,
        ...
        # ── 方案D: 多尺度 TCN ──
        self.temporal_enc = MultiScaleTCNTemporalEncoder(
            window_size=window_size, out_dim=hidden_dim,
            branch_channels=hidden_dim//4, dropout=dropout)
...
# ═══════════════════════════════════════════════════════════════
# 7. ParallelD_USAD (方案D完整模型)
# ═══════════════════════════════════════════════════════════════
class ParallelD_USAD(nn.Module):
