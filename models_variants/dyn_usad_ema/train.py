"""训练: Dynamic USAD + EMA Smoothing (beta sweep)"""
...
from variant_model import DynEMA_USAD
...
def main(beta=0.9):
    ...
    save_dir = os.path.join(cfg["output"]["save_dir"], f"dyn_usad_ema_b{beta:.2f}".replace(".",""))
    ensure_dir(save_dir)
    ...
    model = DynEMA_USAD(
        info["num_variables"], int(cfg["data"]["window_size"]), static_ei, beta,
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch:03d} (P{phase}, beta={beta:.2f}) | train {train_loss:.6f} | val {val_loss:.6f}")
    ...
    save_json({"history": history, "best_val_loss": float(best_val), "beta": beta},
    ...
    metrics = {"beta": beta, "threshold": threshold, "raw": raw_m, "point_adjust": pa_m,
               "train_time_min": train_time/60}
    save_json(metrics, os.path.join(save_dir, "metrics.json"))
    print(f"EMA beta={beta:.2f}: F1={raw_m['f1']:.4f}  AUC={raw_m.get('roc_auc','?')}")
    return metrics

if __name__ == "__main__":
    for b in [0.8, 0.9, 0.95]:
        print(f"\n{'='*60}\n  EMA Smoothing: beta={b}\n{'='*60}")
        main(beta=b)
